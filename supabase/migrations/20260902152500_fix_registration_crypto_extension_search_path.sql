CREATE OR REPLACE FUNCTION create_registration_order(p_role text,p_email text,p_full_name text,p_course_id uuid DEFAULT NULL,p_referral_code text DEFAULT NULL) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, extensions, pg_catalog AS $$

...
export async function createPayment(studentId: string, courseId: string): Promise<Payment> {
  ...
  const { data, error } = await supabase.from('payments').insert({ student_id: studentId, course_id: courseId }).select('*').single()
  if (error) throw new Error(error.message)
...
export async function submitTransaction(paymentId: string, txHash: string): Promise<Payment> {
  ...
  if (!TRON_TX_HASH.test(cleanHash)) throw new Error('Enter a valid 64-character hexadecimal TRON transaction hash.')

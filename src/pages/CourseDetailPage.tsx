...
export default function CourseDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (c) {
        ...
        for (const mod of mods) modsWithLessons.push({ ...mod, lessons: await listLessons(mod.id) })
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-6 border-b border-neutral-200 pb-6">
        <div className="flex items-center gap-2 text-sm text-neutral-600">◷ <span>{course.total_hours} hours</span></div>
        <div className="flex items-center gap-2 text-sm text-neutral-600">☰ <span>{modules.length} modules · {totalLessons} lessons</span></div>
        {totalDuration > 0 && <div className="flex items-center gap-2 text-sm text-neutral-600">◷ <span>{totalDuration} min of content</span></div>}
        <div className="text-sm font-semibold text-neutral-900">{isPaid ? `${course.price} ${course.currency}` : 'Free'}</div>
      ...
      {modules.length === 0 ? <div className="mt-8"><EmptyState title="No modules yet" message="This course does not have any content yet." /></div> : <div className="mt-8 space-y-6">
        {modules.map((mod, mIdx) => <div key={mod.id} className="rounded-lg border border-neutral-200 bg-white">

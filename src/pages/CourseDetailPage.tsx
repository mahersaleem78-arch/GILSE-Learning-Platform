...
export default function CourseDetailPage() {
  ...
  return <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div className="mt-6 flex flex-wrap gap-6 border-b border-neutral-200 pb-6">
      ...
      {totalDuration > 0 && <span className="text-sm text-neutral-500">{totalDuration} min of content</span>}
      <span className="text-sm font-semibold text-neutral-900">{isPaid ? `${course.price} ${course.currency}` : 'Free'}</span>
      {isEnrolled ? <Link to="/dashboard" className="btn-primary ml-auto">Continue learning</Link> : isPaid ? <Link to={user ? `/courses/${course.id}/pay` : `/signup?course=${encodeURIComponent(course.id)}`} className="btn-primary ml-auto">{user ? `Buy for ${course.price} ${course.currency}` : `Register and pay — ${course.price} ${course.currency}`}</Link> : user ? <button onClick={() => void handleFreeEnrollment()} disabled={actionLoading} className="btn-primary ml-auto">{actionLoading ? 'Enrolling…' : 'Start learning'}</button> : <Link to="/signup" className="btn-primary ml-auto">Create account</Link>}
    </div>
    {error && <div className="mt-4"><ErrorState message={error} /></div>}
    {isPaid && !isEnrolled && <div className="mt-4 rounded-lg border border-primary-100 bg-primary-50 p-4 text-sm text-primary-800">Paid courses are unlocked only after the payment is verified on the configured network. New students create their account only after that payment is confirmed.</div>}

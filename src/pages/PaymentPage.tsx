...
export default function PaymentPage() {
  ...
  useEffect(() => {
    if (!user || !id) return
    Promise.all([getCourse(id), getPaymentConfig()])
      .then(([c, cfg]) => { setCourse(c); setConfig(cfg) })
      .catch(e => setError(e instanceof Error ? e.message : 'Unable to load payment details'))
  ...
  const startPayment = async () => {
    ...
    try {
      const p = await createPayment(user.id, course.id)
      setPayment(p)
